...
//using FinAxisLeaseBudgeting.Interfaces;
//using FinAxisLeaseBudgeting.Models;
//using FinAxisLeaseBudgeting.Tools; // Import your DirectAiTools namespace
//using Microsoft.AspNetCore.Mvc;
//using Microsoft.EntityFrameworkCore;
//using Microsoft.Extensions.AI;
//using PlanningAPI.Models;
...
//        private readonly FinAxisDbContext _context;
//        private readonly IUserPropertySecurityRepository _securityRepository;
//        private readonly DirectAiTools _directTools; // Inject Direct Tools class
...
//            FinAxisDbContext context,
//            IUserPropertySecurityRepository securityRepository,
//            DirectAiTools directTools) // Resolve from DI container
//        {
//            _logger = logger;
//            _chatClient = chatClient;
//            _configuration = configuration;
...
//                else
//                {
//                    _logger.LogInformation("Scope-Aware Cache MISS. Executing direct C# tools LLM pipeline.");
//                    finalAiResponse = await ExecuteLiveDirectToolsPipelineAsync(sessionId, request.Prompt, request.UserId, allowedScopeSummary);
//                    source = "live";
...
//        private async Task<string> ExecuteLiveDirectToolsPipelineAsync(string sessionId, string userMessage, long userId, string allowedScopeSummary)
//        {
//            // Bind C# DirectAiTools directly into Microsoft.Extensions.AI framework tools
//            var aiFunctions = new List<AITool>
//            {
//                AIFunctionFactory.Create(_directTools.GetBudgetAssumptionsAsync),
//                AIFunctionFactory.Create(_directTools.GetMarketRentUnitsAsync),
//                AIFunctionFactory.Create(_directTools.GetExpiringLeasesAsync),
//                AIFunctionFactory.Create(_directTools.GetVacantUnitsAsync),
//                AIFunctionFactory.Create(_directTools.GetMasterDataAsync)
//            };
...
//                $"- If an entity has null or empty property/unit mappings, all properties and units under that entity are allowed.\n" +
//                $"- If specific properties/units are mapped, restrict responses and tool outputs strictly to those items.\n" +
//                $"- If There is No mapping of any entity.property/unit, then tell that you dont have access to get any information for any entity/property/unit.\n" +
//                $"- Exclude and filter out any unauthorized entity, property, or unit data completely from the response.\n" +
//                $"=========================================";
...
//            // Execute using native C# tools directly
//            var response = await _chatClient.GetResponseAsync(messages, new ChatOptions { Tools = aiFunctions });
//            return response.Text ?? "No response generated.";
//        }
...
//                    "\n\nDATA REUSE INSTRUCTION: The user is asking a follow-up or filtering question. " +
//                    "Below is the raw dataset previously retrieved from the database in a cached record. " +
//                    "DO NOT call any tools or query the database. Process, filter, or apply assumptions " +
//                    "directly using this provided dataset to conserve tokens and execution time.\n\n" +
//                    $"SECURITY ACCESS SCOPE RESTRICTION:\n{allowedScopeSummary}"),
...
namespace PlanningAPI.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        private readonly FinAxisDbContext _context;
        private readonly IUserPropertySecurityRepository _securityRepository;
        private readonly DirectAiTools _directTools;
        ...
        public async Task<IActionResult> Chat([FromBody] ChatRequest request)
        {
            ...
            try
            {
                ...
                string normalizedQueryTemplate = NormalizeQueryTemplate(request.Prompt);
                string scopeContext = allowedScopeSummary?.Trim() ?? string.Empty;
                string queryHash = ComputeSha256Hash($"{normalizedQueryTemplate}:::{scopeContext}");
                ...
                if (cachedRecord != null)
                {
                    _logger.LogInformation("Scope-Aware Cache HIT for hash: {Hash}", queryHash);
                    finalAiResponse = await ExecuteCachedDataPipelineAsync(request.Prompt, cachedRecord.AssistantResponse, allowedScopeSummary);
                    source = "scope-aware-cache";
                }
                else
                {
                    _logger.LogInformation("Scope-Aware Cache MISS. Executing live tools pipeline.");
                    finalAiResponse = await ExecuteLiveDirectToolsPipelineAsync(sessionId, request.Prompt, request.UserId, allowedScopeSummary);
                    source = "live";
                }

                // Dynamically evaluate if the response indicates unsupported capability/failure
                bool isUnsupportedOrError = IsUnsupportedOrErrorResponse(finalAiResponse);

                // ONLY save valid responses to ChatHistories
                if (!isUnsupportedOrError && request.SessionId == null)
                {
                    var newHistoryEntry = new ChatHistoryMessage
                    {
                        UserId = request.UserId,
            ...
            {
                _logger.LogError(ex, "Tool execution error occurred while processing query.");

                // Return fallback message listing available tools WITHOUT saving to ChatHistories
                return Ok(new
                {
                    success = false,
                    sessionId = sessionId,
                    response = GetMissingToolOrErrorFallback(),
                    source = "live"
                });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unhandled error processing chat message.");
        ...
        private async Task<string> ExecuteLiveDirectToolsPipelineAsync(string sessionId, string userMessage, long userId, string allowedScopeSummary)
        {
            ...
            string dynamicSystemPrompt = $"""
                ...
                {ToolCatalog.GetFormattedToolCatalog()}

                UNSUPPORTED QUERY HANDLING RULE:
                - If the user asks for data or actions for which NO tool is registered in the capability list above (e.g., creating user accounts, weather data, unsupported metrics), DO NOT attempt to fabricate an answer.
                - State clearly: "I cannot process this request because no tool is currently available for this data."
                - Immediately follow up by displaying the list of available tools, what they do, and example prompts to assist the user.
            ...
            var messages = new List<ChatMessage>
            {
                new ChatMessage(ChatRole.System, dynamicSystemPrompt)
            };
            ...
            var response = await _chatClient.GetResponseAsync(messages, new ChatOptions { Tools = aiFunctions });
            return response.Text ?? GetMissingToolOrErrorFallback();
        }
        ...
        private async Task<string> ExecuteCachedDataPipelineAsync(string userPrompt, string previousCachedDatasetResponse, string allowedScopeSummary)
        {
            ...
            {
                new ChatMessage(ChatRole.System, AiPrompt.SystemPrompt +
                    "\n\nDATA REUSE INSTRUCTION: Use the cached dataset provided below to answer user queries when possible. " +
                    "If the query requires completely unsupported functionality, inform the user and show available tools.\n\n" +
                    $"SECURITY ACCESS SCOPE RESTRICTION:\n{allowedScopeSummary}"),
        ...
        private static bool IsUnsupportedOrErrorResponse(string responseText)
        {
            if (string.IsNullOrWhiteSpace(responseText)) return true;

            string[] failurePhrases = new[]
            {
                "no tool is currently available",
            ...
            return failurePhrases.Any(phrase => responseText.Contains(phrase, StringComparison.OrdinalIgnoreCase));
        }

        private static string GetMissingToolOrErrorFallback()
        {
            return "I cannot process this request because no tool is available for this functionality.\n\n" +
                   ToolCatalog.GetFormattedToolCatalog();
        }
        ...
        {
            string normalized = rawPrompt.Trim().ToLowerInvariant();
            return Regex.Replace(normalized, @"\b\d+\b", "{value}");
        }
